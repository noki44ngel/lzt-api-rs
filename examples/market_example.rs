...
async fn main() -> ApiResult<()> {
    ...
    let client = ApiClient::builder().base_url("https://api.lzt.market").token(&token).retry(3).build()?;
    ...
    match market.get_stats().await {
        Ok(s) => println!("  Items: {}, Sold: {}, Views: {}", s.total_items, s.total_sold, s.total_views),
        Err(e) => eprintln!("  Error: {}", e),
    }
